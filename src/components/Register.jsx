...
function Register() {
  ...
  const requiredDetections = 45; // Frames required for registration
  ...
  const initMediaPipe = async () => {
    try {
      ...
      setMessage({ type: 'danger', text: 'Failed to initialize face detection' });
      setScanning(false);
    }
  };
  ...
  // Capture the best frame for registration
  const captureBestFrame = () => {
    if (webcamRef.current) {
      ...
      if (imageSrc) {
        ...
        // Convert base64 to file for registration
        fetch(imageSrc)
          .then(res => res.blob())
          .then(blob => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedImage) {
      setMessage({ type: 'danger', text: 'Please complete face scanning first' });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: 'success', text: `User ${result.name} registered successfully!` });
      ...
      setMessage({
        type: 'danger',
        text: error.response?.data?.detail || 'Error registering user'
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="col-md-9 col-lg-7">
          <div className="glass-panel p-4 p-md-5">
            <div className="text-center mb-4">
              <h3 className="text-gradient fw-bold mb-2">Secure Registration</h3>
              <p className="text-muted small">AI-powered facial recognition enrollment</p>
            </div>

            {message.text && (
              <div className={`alert ${message.type === 'danger' ? 'bg-danger text-white border-danger' : 'bg-success text-white border-success'} bg-opacity-25 border rounded-3`} role="alert">
                <i className={`bi ${message.type === 'danger' ? 'bi-exclamation-triangle' : 'bi-check-circle'} me-2`}></i>
                {message.text}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                <label htmlFor="name" className="form-label text-muted fw-medium small text-uppercase tracking-wide">Full Name</label>
                <div className="input-group">
                  <span className="input-group-text bg-dark bg-opacity-50 border-secondary border-opacity-25 text-muted">
                  ...
                  <input
                    ...
                    placeholder="Enter employee or student name"
                    required
                  />
                </div>
              </div>
              ...
              {scanComplete && (
                <div className="mt-4 pt-2 border-top border-secondary border-opacity-25 text-center">
                  <button
                    type="button"
                  ...
                  <button
                    type="submit"
                    className="btn-premium btn-premium-success px-4"
                    disabled={loading}
                  >
                    {loading ? (
                      <>
                      ...
                      <><i className="bi bi-check2-circle"></i> Complete Enrollment</>
                    )}
                  </button>
                </div>
              )}
